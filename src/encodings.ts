...
export function strToBuffer(input: string, fromPlain: boolean = true) {
  if (input.startsWith('hex:')) return { encoding: 'hex', data: hexToBuffer(input.slice('hex:'.length)) }
...
const getCustomB64Encoder = ([
  ...
  slashChar,
  equalChar
]: string[]): Encoder => (buffer: Buffer) => buffer
      .toString('base64')
      .replace(/\+/g, plusChar || '')
      .replace(/\//g, slashChar || '')
      .replace(/=/g, equalChar || '')
const encodings: { [key: string]: Encoder } = {
  ...
  'base64-urlsafe': (x: Buffer) => x.toString('base64').replace(/\+/g, '-').replace(/\//g, '_'),

...
const OracleCaller = (options: OracleCallerOptions) => {
  ...
  const injectionStringMissing = !_url.includes(POPAYLOAD)
    && !String(
      typeof headers === 'object' ? JSON.stringify(headers) : headers
    ).includes(POPAYLOAD)
    && !(data || '').includes(POPAYLOAD)
  if (injectionStringMissing) {
    console.warn(
      chalk`{yellow.underline Warning}: ${POPAYLOAD} not found in url, headers, or data. This may cause unexpected behavior. The string will be appended to the end of the url.\n`
    )
  }
  const networkStats = {
    count: 0,
  ...
  async function callOracle(payload: Buffer): Promise<OracleResult> {
    ...
    const url = (
      injectionStringMissing ? _url + payloadString : addPayload(_url)
    ) as string

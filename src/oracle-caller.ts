...
const OracleCaller = (options: OracleCallerOptions) => {
  ...
  ow(predicateString, "predicateString", ow.string);
  ...
  if (requestOptions.headers)
    ow(requestOptions.headers, ow.any(ow.object, ow.string, ow.array));
  ow(requestOptions.data, ow.optional.string);

  const { method, headers, data } = requestOptions;
  const injectionStringPresent =
    !_url.includes(POPAYLOAD) &&
    !String(
  ...
  async function callOracle(payload: Buffer): Promise<OracleResult> {
    ...
    const addPayload: AddPayload = (str) =>
      str ? str.replace(injectionRegex, payloadString) : str;
    const url = (

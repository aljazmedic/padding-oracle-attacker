...
const PaddingOracle = (options: PaddingOracleOptions) => {
  ...
  async function processByte({
    ...
    const { twoBlocks } = constructPayload({
      blockI,
      byteI,
      byte,
      currentPadding
    ...
    for (const byteI of range(blockSize - 1, -1)) {
      ...
      if (concurrency > 1) {
        ...
        for (const byte of byteRange) {
          const success = await processByte({
            blockI,
            byteI,
            byte,
            currentPadding,
      ...
      if (!foundOffsets.has(offset)) {
        throw Error(
          `Padding oracle failure for offset: 0x${offset.toString(
            16
          )}. Try again or check the parameter you provided for determining decryption success.`
        )
      }
  ...
  async function processBlocks() {
    const blockIndexes = startFromFirstBlock
      ? range(blockCount - 1)
      : range(blockCount - 2, -1)
